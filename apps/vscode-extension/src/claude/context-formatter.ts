...
export class ContextFormatter {
  formatWorkspaceContext(workspace: WorkspaceInfo): string {
    ...
    if (
      workspace.projectStructure?.frameworks &&
      workspace.projectStructure.frameworks.length > 0
    ) {
      sections.push(
        `**Frameworks**: ${workspace.projectStructure.frameworks.join(', ')}`,
      );
    }
    ...
    if (workspace.activeFile) {
      sections.push(
        `**Active File**: ${workspace.activeFile.path} (${workspace.activeFile.language})`,
      );
    }
    ...
    const formatted = elements
      .map((element, index) => {
        ...
        if (element.children && element.children.length > 0) {
          const childTypes = [
            ...new Set(element.children.map((c) => c.tagName)),
          ];
          parts.push(
            `**Children**: ${element.children.length} elements (${childTypes.join(', ')})`,
          );
        }
  ...
  formatPluginContext(plugins: PluginContextData[]): string {
    ...
    for (const [type, pluginList] of Object.entries(grouped)) {
      ...
      const pluginData = pluginList
        .map((plugin: PluginContextData) => {
  ...
  private groupPluginsByType(
    plugins: PluginContextData[],
  ): Record<string, PluginContextData[]> {
