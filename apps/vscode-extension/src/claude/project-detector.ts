...
import { PackageInfo } from './workspace-types';
...
export class ProjectDetector {
  ...
  async detectProjectType(): Promise<string[]> {
    ...
    for (const check of checks) {
      if (check.pattern) {
        ...
        const files = await vscode.workspace.findFiles(`**/${check.file}`, '**/node_modules/**', 1);
        if (files.length > 0) {
  ...
  async detectFrameworks(): Promise<string[]> {
    ...
    if (workspaceRoot) {
      // Angular
      if (await this.fileExists(vscode.Uri.joinPath(workspaceRoot, 'angular.json'))) {
        if (!frameworks.includes('angular')) frameworks.push('angular');
      }
      
      // Vue
      const vueFiles = await vscode.workspace.findFiles('**/*.vue', '**/node_modules/**', 1);
      if (vueFiles.length > 0 && !frameworks.includes('vue')) {
        frameworks.push('vue');
      }
      
      // Next.js
      if (await this.fileExists(vscode.Uri.joinPath(workspaceRoot, 'next.config.js')) ||
          await this.fileExists(vscode.Uri.joinPath(workspaceRoot, 'next.config.ts'))) {
  ...
  async getDependencies(): Promise<string[]> {
    ...
    const ignoreDeps = [
      '@types/', 'eslint-', 'babel-', 'webpack-', '@babel/',
      'postcss-', 'autoprefixer', 'prettier', 'husky', 'lint-staged'
    ];
  ...
  private async readPackageJson(): Promise<any | undefined> {
    // Check cache
    if (this.packageJsonCache && Date.now() - this.packageJsonCache.timestamp < this.CACHE_DURATION) {
