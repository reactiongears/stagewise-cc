...
import { DEFAULT_CONFIGURATION, CONFIGURATION_METADATA } from './config-defaults';
...
export class ClaudeConfigService extends EventEmitter {
  ...
  async updateConfiguration(
    ...
    const validation = this.validateConfiguration({ ...this.currentConfig, ...updates });
    if (!validation.isValid) {
      throw new Error(`Invalid configuration: ${validation.errors.map(e => e.message).join(', ')}`);
    }
    
    const target = scope === ConfigurationScope.WORKSPACE 
      ? vscode.ConfigurationTarget.Workspace 
      : vscode.ConfigurationTarget.Global;
    
    const config = vscode.workspace.getConfiguration(ClaudeConfigService.CONFIG_PREFIX);
  ...
  validateConfiguration(config: Partial<ClaudeConfiguration>): ConfigurationValidationResult {
    ...
    if (config.contextWindow?.maxSize !== undefined) {
      if (config.contextWindow.maxSize < 1000 || config.contextWindow.maxSize > 200000) {
        errors.push({
          field: 'contextWindow.maxSize',
          message: 'Context window size must be between 1,000 and 200,000 characters',
    ...
    if (config.telemetry?.enabled) {
      warnings.push('Telemetry is enabled. Your usage data will be collected anonymously.');
    }
  ...
  async resetConfiguration(scope: ConfigurationScope = ConfigurationScope.WORKSPACE): Promise<void> {
    const target = scope === ConfigurationScope.WORKSPACE 
      ? vscode.ConfigurationTarget.Workspace 
      : vscode.ConfigurationTarget.Global;
    ...
    for (const key of Object.keys(CONFIGURATION_METADATA)) {
      const configKey = key.replace(`${ClaudeConfigService.CONFIG_PREFIX}.`, '');
  ...
  async saveProfile(name: string, description?: string): Promise<void> {
    ...
    await this.context.globalState.update(ClaudeConfigService.PROFILES_KEY, profiles);
    
    this.log(`Configuration profile '${name}' saved`);
    vscode.window.showInformationMessage(`Configuration profile '${name}' saved successfully`);
  }
  ...
  async loadProfile(name: string): Promise<void> {
    ...
    await this.updateConfiguration(profile.config, ConfigurationScope.WORKSPACE);
    this.log(`Configuration profile '${name}' loaded`);
    vscode.window.showInformationMessage(`Configuration profile '${name}' loaded successfully`);
  }
  ...
  async importConfiguration(data: string): Promise<void> {
    try {
      ...
      await this.updateConfiguration(importData.configuration, ConfigurationScope.WORKSPACE);
      
      if (importData.profiles) {
        await this.context.globalState.update(ClaudeConfigService.PROFILES_KEY, importData.profiles);
      }
  ...
  private flattenKey(key: string, value: any, prefix = ''): Record<string, any> {
  ...
  private getChangedKeys(oldConfig: any, newConfig: any, prefix = ''): string[] {
    ...
    for (const key of allKeys) {
      ...
      if (oldConfig[key] !== newConfig[key]) {
        if (typeof oldConfig[key] === 'object' && typeof newConfig[key] === 'object') {
          changes.push(...this.getChangedKeys(oldConfig[key], newConfig[key], fullKey));
        } else {
