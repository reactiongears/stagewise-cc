 ...
 */
export function validateDOMElementData(element: any): element is DOMElementData {
  return (
    element &&
 ...
 */
export function createDOMElementData(tagName: string, selector: string): DOMElementData {
  return {
    tagName,
    selector,
    attributes: {},
...
export function isInteractiveElement(element: DOMElementData): boolean {
  ...
  const hasClickHandler = !!element.attributes.onclick || element.metadata.hasEventListeners;
  const hasRole = element.attributes.role === 'button' || element.attributes.role === 'link';
 ...
 */
export function serializeDOMElement(element: DOMElementData, compact: boolean = true): string {
  if (compact) {
