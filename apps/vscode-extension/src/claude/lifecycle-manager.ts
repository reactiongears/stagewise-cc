...
export class LifecycleManager {
  ...
  async activate(): Promise<void> {
    ...
    try {
      ...
      const configService = new ClaudeConfigService(this.context, outputChannel);
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      if (authService.getStatus() === 'valid') {
        ...
        logger.warning('Skipping Claude service initialization - authentication not configured');
  ...
  private registerCommands(): void {
    const { authService, configService, claudeService, outputChannel } = this.state.components;
    
    if (!authService || !configService || !claudeService || !outputChannel) {
      throw new Error('Required components not available for command registration');
    }
    ...
    const restartCommand = vscode.commands.registerCommand(
      ...
      async () => {
        try {
          ...
          await this.restart();
          vscode.window.showInformationMessage('Claude service restarted successfully');
        } catch (error) {
  ...
  private showWelcomeMessage(): void {
    const message = 'Welcome to Claude Code! Get started by setting your API key.';
    vscode.window.showInformationMessage(
      message,
      'Set API Key',
      'Open Documentation'
...
export function createLifecycleManager(context: vscode.ExtensionContext): LifecycleManager {
