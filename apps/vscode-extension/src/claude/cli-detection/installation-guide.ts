 ...
 */
function getGenericGuidance(): InstallationGuidance {
  return {
...
export function getGuidance(
  ...
  const guides: Record<NodeJS.Platform, InstallationGuidance> = {
    ...
    aix: getGenericGuidance(),
    android: getGenericGuidance(),
    freebsd: getGenericGuidance(),
    haiku: getGenericGuidance(),
    openbsd: getGenericGuidance(),
    sunos: getGenericGuidance(),
    cygwin: getGenericGuidance(),
    netbsd: getGenericGuidance(),
  };

  return guides[platform] || getGenericGuidance();
}

/**
 * Get quick install command for platform
 */
export function getQuickInstallCommand(
  platform: NodeJS.Platform = os.platform(),
): string {
  switch (platform) {
    case 'darwin':
 ...
 */
export function getMarkdownGuide(platform?: NodeJS.Platform): string {
  const guidance = getGuidance(platform);
  const lines: string[] = [
    `# ${guidance.title}`,
    '',
    '## Installation Steps',
 ...
 */
export function getHtmlGuide(platform?: NodeJS.Platform): string {
  const guidance = getGuidance(platform);
...
/**
 * Re-export namespace for backward compatibility
 */
export const CLIInstallationGuide = {
  getGuidance,
