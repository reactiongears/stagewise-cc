...
import type { Session, SessionMetadata, SessionConfig } from './session-types';
...
export class SessionIntegration {
   ...
   * Create a new branch
   */
  async createBranch(name?: string, fromTurnId?: string): Promise<void> {
  ...
  private setupEventHandlers(): void {
    ...
    if (this.config.enableBranching) {
      this.branching.onBranchNavigation((event) => {
        this.logger.debug(
          `Branch navigation: ${event.fromBranchId} -> ${event.toBranchId}`,
        );
      });
    }
  }
