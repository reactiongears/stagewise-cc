...
import { SYSTEM_PROMPT_TEMPLATE, buildWorkspaceSection, buildDOMSection, buildFileSection, buildPluginSection } from './prompt-templates';
...
export class PromptTransformer extends EventEmitter {
  ...
  constructor() {
    ...
    this.outputChannel = vscode.window.createOutputChannel('Claude Prompt Transformer');
  ...
  buildSystemPrompt(context: ClaudePromptContext): string {
    ...
    if (context.workspaceMetadata) {
      const workspaceOverview = this.formatter.formatWorkspaceContext(context.workspaceMetadata);
  ...
  async buildUserPrompt(context: ClaudePromptContext): Promise<string> {
    ...
    if (context.strategy !== 'minimal' && context.workspaceMetadata?.projectStructure) {
      const relatedFiles = this.selectRelatedFiles(context);
      if (relatedFiles.length > 0) {
        const relatedContent = await this.formatter.formatFileContent(relatedFiles);
        sections.push({
          type: 'related_files',
    ...
    if (context.pluginContexts && context.pluginContexts.length > 0) {
      const pluginContent = this.formatter.formatPluginContext(context.pluginContexts);
      sections.push({
        type: 'plugin_context',
    ...
    const tokenBudget = this.tokenManager.calculateTokenBudget(context.maxTokens || 100000);
    const userPromptBudget = tokenBudget.userPrompt;
    const trimmedSections = this.tokenManager.trimToFit(sections, userPromptBudget);
  ...
  private applyContextStrategy(sections: PromptSection[], strategy: ContextStrategy): PromptSection[] {
    switch (strategy) {
      case 'minimal':
