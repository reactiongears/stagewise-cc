...
export class ModificationAnalyzer {
  ...
  async analyzeOperation(operation: FileOperation): Promise<AnalysisResult> {
    ...
    const result: AnalysisResult = {
      ...
      suggestions,
      requiresReview:
        risk !== RiskLevel.LOW || impacts.some((i) => i.severity === 'high'),
    };

    this.logger.debug(
      `Analyzed operation ${operation.id}: Risk=${risk}, Impacts=${impacts.length}`,
    );
    return result;
  }
  ...
  suggestAlternatives(operation: FileOperation): Alternative[] {
    ...
    if (
      ...
      this.estimateRisk(operation) === RiskLevel.HIGH
    ) {
      alternatives.push({
   ...
   */
  private async analyzeCreateOperation(
    operation: FileOperation,
  ): Promise<Impact[]> {
    const impacts: Impact[] = [];
   ...
   */
  private async analyzeUpdateOperation(
    operation: FileOperation,
  ): Promise<Impact[]> {
    const impacts: Impact[] = [];
   ...
   */
  private async analyzeDeleteOperation(
    operation: FileOperation,
  ): Promise<Impact[]> {
    const impacts: Impact[] = [];
  ...
  private analyzeCodeQuality(operation: FileOperation): Impact[] {
    ...
    if (
      ...
      operation.metadata?.language === 'typescript'
    ) {
      impacts.push({
        type: 'style',
        severity: 'low',
  ...
  private generateSuggestions(
    ...
    if (impacts.some((i) => i.type === 'api')) {
      ...
      suggestions.push(
        'Consider adding deprecation notices for gradual migration',
      );
    }
  ...
  private async findRelatedTestFiles(filePath: string): Promise<string[]> {
    ...
    for (const pattern of testPatterns) {
      try {
        ...
        files.forEach((file) =>
          testFiles.add(vscode.workspace.asRelativePath(file)),
        );
      } catch {
        // Ignore errors
      }
