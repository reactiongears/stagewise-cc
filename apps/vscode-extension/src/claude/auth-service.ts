...
import { AuthCredentials, AuthStatus, AuthValidationResult, AuthStatusChangeEvent } from './auth-types';
import {
  API_KEY_SECRET_KEY,
...
export class ClaudeAuthService extends EventEmitter {
  ...
  async deleteApiKey(): Promise<void> {
    try {
      ...
      vscode.window.showInformationMessage('Claude API key removed successfully.');
    } catch (error) {
    ...
    for (let attempt = 1; attempt <= VALIDATION_RETRY_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt === VALIDATION_RETRY_ATTEMPTS) {
          return {
            isValid: false,
            error: error instanceof Error ? error.message : ERROR_MESSAGES.VALIDATION_FAILED
          };
        }
  ...
  private async performValidation(apiKey: string): Promise<AuthValidationResult> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          isValid: false,
          error: (errorData as any).error?.message || `Validation failed with status ${response.status}`
        };
      }
    } catch (error) {
      clearTimeout(timeout);
  ...
  private async migrateFromLegacyStorage(): Promise<void> {
    ...
    try {
      ...
      const workspaceConfig = vscode.workspace.getConfiguration('stagewise.claude');
      ...
      if (workspaceKey && this.isValidKeyFormat(workspaceKey)) {
        ...
        await workspaceConfig.update('apiKey', undefined, vscode.ConfigurationTarget.Workspace);
        vscode.window.showInformationMessage('Claude API key migrated to secure storage');
      }
      ...
      if (userKey && userKey !== workspaceKey && this.isValidKeyFormat(userKey)) {
        this.log('Found API key in user settings, migrating...');
        await this.setApiKey(userKey);
        await userConfig.update('apiKey', undefined, vscode.ConfigurationTarget.Global);
      }
      ...
      if (envKey && this.isValidKeyFormat(envKey)) {
        ...
        if (!currentKey) {
          ...
          vscode.window.showInformationMessage('Claude API key from environment variable stored securely');
        }
      }
  ...
  private updateStatus(newStatus: AuthStatus): void {
    ...
    this.log(`Authentication status changed: ${previousStatus} -> ${newStatus}`);
  }
  ...
  private handleError(message: string, error: any): void {
    ...
    if (error?.message && Object.values(ERROR_MESSAGES).includes(error.message)) {
      ...
      vscode.window.showErrorMessage(`${message}. Check the output channel for details.`);
    }
  }
  
  private log(message: string, level: 'info' | 'warning' | 'error' = 'info', maskSensitive = false): void {
    const timestamp = new Date().toISOString();
    const logMessage = `[${timestamp}] [AUTH] [${level.toUpperCase()}] ${message}`;
    
    // Mask sensitive information in logs
    const sanitizedMessage = maskSensitive ? logMessage.replace(/sk-ant-[\w-]+/g, 'sk-ant-***') : logMessage;
