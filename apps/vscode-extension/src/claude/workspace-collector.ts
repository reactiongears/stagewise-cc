...
export class WorkspaceCollector {
  ...
  private workspaceCache: { data: WorkspaceInfo; timestamp: number } | null =
    null;
  ...
  async gatherWorkspaceInfo(): Promise<WorkspaceInfo> {
    ...
    if (
      this.workspaceCache &&
      Date.now() - this.workspaceCache.timestamp < this.CACHE_DURATION
    ) {
      return this.workspaceCache.data;
    }

    const folders = vscode.workspace.workspaceFolders || [];
    const workspaceFolders: WorkspaceFolder[] = folders.map(
      (folder, index) => ({
        name: folder.name,
